...
tmesh_path = '/Volumes/flamingo_data/_GCAM/GCAM_20230516/matlab_output_w_fused/target-mesh-port-rotp90.json'
data_dir = '/Volumes/flamingo_data-1/_Salman/_calibration/Jan23_loft/dataset_may11'
bin_dir = '/Users/salman_naqvi/Documents/Project-Display/t288_display_incubation/platform-calibration'
subdirs = ['p.0p0p0_p0p0p0'] #[name for name in os.listdir(data_dir) if name not in ['pattern', '.DS_Store']]
...
for subdir in subdirs:
    ...
    cmd_fit = f'{bin_dir}/bin/lens-model-tools fit-model-omfc -r={data_path}/rays-cp90.rays -e=0 -c=1 -n=20 -o={data_path}/left.omfc'
...
for subdir in subdirs:
    data_path = f'{data_dir}/{subdir}'
    cmd_frg = f'{bin_dir}/bin/lens-model-tools fit-rigid-parts-omfc -r={data_path}/rays-cp90.rays -m={data_dir}/p.0p0p0_p0p0p0/left-omfc.json -n=20 -e=0 -o={data_path}/left-rgd.omfc'
    out = subprocess.run(cmd_frg, shell=True, capture_output=True, text=True)
    if out.stderr:
        raise Exception(out.stderr)
    print(out.stdout)

    cmd_val = f'{bin_dir}/bin/lens-model-tools validate-model-omfc -m={data_path}/left-omfc-rgd.json -e=0 -r={data_path}/rays-cp90.rays -j={data_path}/error-left-rgd-omfc.json'
    out = subprocess.run(cmd_val, shell=True, capture_output=True, text=True)
    if out.stderr:
        if not re.search('Validation complete', out.stdout):
            raise Exception(out.stderr)
